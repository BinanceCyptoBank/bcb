...
// -------------------- -------------------- COMMUNITY - PAGE -------------------- --------------------
    // -------------------- Add members to forgot password page --------------------
        function communityNumber() {
            ...
            firebase.database().ref('member').on('value', function(element) {
                firebasefiledir = element.val();
                var clientPrint = "";
                var memberPrint = "";

                for (let i in firebasefiledir) {
                    clientPrint += `
                        <div class="client-select-name-main-wrap client-page-count-print hide-date-display" id="${i}">
                            <div class="container">
                ...
                document.getElementById("owner-member-list-container").innerHTML = clientPrint;
                document.getElementById("member-list-container").innerHTML = memberPrint;
                var displayClient = document.getElementsByClassName("client-page-count-print");
                var displayMember = document.getElementsByClassName("member-page-count-print");
                var displayMemberLength = document.getElementsByClassName("member-page-count-print").length;
                var memberNumber = Number(members) * Number(displayMemberLength);
                ...
                for (let i = 0; i < displayClient.length; i++) {
                    if(displayClient[i].id === userUid){
                        displayClient[i].classList.remove("hide-date-display")
                    } 
                }
    ...
    // -------------------- Message Page container --------------------
        ...
        window.setInterval(() => { 
            firebase.database().ref('post').on('value', function(snapshot) {
                ...
                for (let i in chatDatabase) {
                    chatPrint += `
                        ...
                        <div class="comm-message comm-post chat-client-display" id="${chatDatabase[i].date}" onclick="postClickContainer(this)" data-message-id="${i}">
                            <div>
                                <div class="post-img"><img src="${chatDatabase[i].posturl}" class="img-fluid"></div>
                                <div class="container">
                                    <div class="post-txt">
                                ...
                                <div class="container">
                                    <div class="receive-send-link-btn comm-post-link-btn">
                                        <div class="receive-send-left-wrap" style="margin-right: 0.75em;align-items: center;">
                                            ...
                                            <span class="comm-comment-txtlink one-line">Comments</span>
                                        </div>
            ...
            // Display Chat container per Date
            var chatDisplay = document.getElementsByClassName("chat-client-display");
            for (let i = 0; i < chatDisplay.length; i++) {
                const element = chatDisplay[i].id;
                var currentDate = new Date();
                var expiry = new Date(`${element}`);
                if (currentDate.getTime() >= expiry.getTime()) {
                    chatDisplay[i].style.display = "flex"
    ...
    // -------------------- --------------------
        ...
        function postClickContainer(response) {
            ...
            firebase.database().ref(`post/${empId}`).on('value', snapshot => {
                ...
                document.getElementById("comment-post-img-sub").innerHTML = `
                    <img src="${chatDatabase.posturl}" class="img-fluid">
                `;
                //
        ...
        function addCommentToConversation(commentDatabase) {
            ...
            // Display Chat container per Date
            var chatDisplay = document.getElementsByClassName("comment-client-display");
            for (let i = 0; i < chatDisplay.length; i++) {
                const element = chatDisplay[i].id;
                var currentDate = new Date();
                var expiry = new Date(`${element}`);
                if (currentDate.getTime() >= expiry.getTime()) {
                    chatDisplay[i].style.display = "flex"
                    chatDisplay[i].classList.add("comment-count-class")
                } else if (currentDate.getTime() < expiry.getTime()) {
                    chatDisplay[i].style.display = "none"
                }
            }
